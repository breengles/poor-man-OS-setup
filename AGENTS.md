...
.pi/
  agent/
    ...
    extensions/            # pi extensions (footer, /context, web-search-active)
...
- **Web access** comes from the `pi-web-access` package, but its built-in `web_search` is disabled
  (`tools.webSearch.enabled: false` in `.pi/web-search.json`). `.pi/agent/extensions/web-search-active.ts` re-registers
  `web_search`, reusing pi-web-access's `search()` and `generateSummaryDraft()` building blocks with the currently
  active model as the summary model. That file is tracked here, so API keys must go in the environment, never in it.
