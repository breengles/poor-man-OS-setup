...
- **Conditionally alias commands** only when available; guard aliases for AI agents with `[ -z "$AGENT" ] && [ -z "$CLAUDECODE" ]`
...
8. Guard shell aliases with `[ -z "$AGENT" ] && [ -z "$CLAUDECODE" ]` when they might confuse AI agents
