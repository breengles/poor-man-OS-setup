...
- Run: `grep -rn "TBD" <changed-files>` and similarly for TODO, FIXME, HACK, XXX
...
- Run: `grep -rn -i "password" <changed-files>` and similarly for api_key, secret, token
...
- Assertions are meaningful (not asserting true everywhere).
...
- **No function-local / method-local imports.** Grep the diff for `import`
  statements that are indented inside a function or method body. These almost
  always exist to dodge a circular import and are an anti-pattern: they signal
  either a broken module dependency graph or the implementer cutting corners. If
  ...
  lazy-loaded optional dependencies (e.g. heavy libraries gated behind
  a feature flag) and must be justified in the implementer's CONCERNS.
