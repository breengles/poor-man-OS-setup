...
Document the image dataset at the path given as the argument.

## Safety

**The only file you may create or edit is `install.md` or `install-new.md` in the dataset root.**

Do not create, modify, move, rename, or delete any other file in the dataset, and write no temporary files inside it -- keep
scratch work in a temporary directory outside the dataset. All inspection is read-only: open images to read dimensions, read JSONs, load `.npz`
