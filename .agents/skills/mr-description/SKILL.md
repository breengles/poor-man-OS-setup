...
**Generate mode** (invoked with no argument): analyze the current branch against its base (`main`, fallback `master`) and
output the title and description as one copy-pasteable message. Modify no MR.

**Update mode** (invoked with an MR URL or IID -- `123`, `!123`, or a `/-/merge_requests/<iid>` URL): resolve the MR
