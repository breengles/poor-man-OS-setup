---
name: todo-implement
description: Implement items from a TODO file, one at a time, via an independent implementer subagent per item. The main session acts as orchestrator only.
...
item you dispatch a fresh **todo-implementer** subagent that writes the code. You
handle TODO-file reading, item sequencing, committing, and updating `todos/<area>.md`.
...
A **batch** is one or more items committed together. Most batches are
...
- **One combined implementer.** If the items are semantically
...
"Independent" / "parallel" here only authorizes batching of commits -- it
...
completed batch, retain only a **one-line summary** (e.g. "solver#5: 2 files
changed, commit abc1234"; or "solver batch [#5,#7]: commit abc1234") and discard the full subagent reports from your working memory.
...
they noticed, you will file those as new TODO items in step 3d.
...
### 3c. Update the TODO file

Run this step **only after** every implementer in the batch has returned
`COMPLETE`. This marks the item `Done` as a **transient** state -- it records
what shipped so the wrap-up doc-reconciliation (Step 4) knows what changed, and the item
is removed from the file at wrap-up. For every completed item in the batch:
...
edits from step 3c. The TODO update and the implementation changes go in the
...
- The "Suggested resolution order" was already pruned in step 3c; confirm no removed item
  lingers there.
...
- **Resolved items are removed, not retained.** Do not keep a `Done` ledger. `Done` is a
  transient state that exists only between step 3c and the Step 4 purge; git history is
...
- **Sequential dispatch.** Never dispatch multiple implementers simultaneously --
  ...
  "Parallel" / "independent" items only authorize batched committing,
...
- **Context hygiene.** After each item, retain only the one-line summary. Do NOT
  carry forward the full implementer reports into the next item's context.
